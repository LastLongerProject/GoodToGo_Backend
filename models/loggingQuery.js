...
module.exports = {
    ...
    imageLog: function(req, res, payload, next) {
        process.nextTick(function() {
            newLogging = new Logging({
                'ip': req.connection.remoteAddress,
                'url': req.url,
            ...
            newLogging.save(function(err) {
                ...
                if (typeof payload === 'undefined') {
                    return res.status(401).json({ type: 'loggingImage', message: 'JWT Invalid' });
                } else if (typeof payload.iat === 'undefined') {
                    return res.status(401).json({ type: 'loggingImage', message: 'Token Invalid' });
                } else { return next(); }
            });
        });
    },
