        ...
        return new Promise(function (resolve, reject) {
            tradeDB.find({
                ...
                trades_Of_Used_Container = trades_Of_Used_Container.map(trade => [
                    ...
                    trade.tradeTime.toLocaleDateString('roc', {
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit'
                    }),
                    trade.tradeTime.toLocaleTimeString('roc', {
                        hour: '2-digit',
                        minute: '2-digit',
                        second: '2-digit'
                    })
                ...
                orders.forEach(order => {
                    if (order.containerID) {
                        NotReturn_orders.push(
                            [
                                order.orderTime.toLocaleDateString('roc', {
                                    year: 'numeric',
                                    month: '2-digit',
                                    day: '2-digit'
                                }),
                                order.orderTime.toLocaleTimeString('roc', {
                                    hour: '2-digit',
                                    minute: '2-digit',
                                    second: '2-digit'
                                }),
        ...
        return new Promise(function (resolve, reject) {
            ContainerDB.find({
                ...
                let Containers_in_Status_1 = containers.map(container => [
                    container.lastUsedAt.toLocaleDateString('roc', {
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit'
                    }),
                    container.lastUsedAt.toLocaleTimeString('roc', {
                        hour: '2-digit',
                        minute: '2-digit',
                        second: '2-digit'
                    }),
                    ...
                    trades.forEach(trade => {
                        if (trade.tradeType.oriState === 1 && trade.tradeType.newState === 3) {
                            if (trade.oriUser.storeID === trade.newUser.storeID) {
                                the_Date_Return_To_OriStore.push(trade.tradeTime.toLocaleDateString('roc', {
                                    year: 'numeric',
                                    month: '2-digit',
                                    day: '2-digit'
                                }));
                            } else if (trade.newUser.phone.slice(0, 3) === 'bot') {
                                the_Date_Return_To_Bot.push(trade.tradeTime.toLocaleDateString('roc', {
                                    year: 'numeric',
                                    month: '2-digit',
                                    day: '2-digit'
                                }))
                            } else {
                                the_Date_Return_To_Other_Store.push(trade.tradeTime.toLocaleDateString('roc', {
                                    year: 'numeric',
                                    month: '2-digit',
                                    day: '2-digit'
                                }))
                        ...
                        while (Container_Trades.length >= 2) {
                            ...
                            if (Trade[0] === 'Rent') {
                                ...
                                if (Rent_Trade[1] === Return_Trade[1]) {
                                    the_Date_Return_To_OriStore.push(Rent_Trade[3].toLocaleDateString('roc', {
                                        year: 'numeric',
                                        month: '2-digit',
                                        day: '2-digit'
                                    }));
                                } else if (Return_Trade[2] !== 'bot') {
                                    the_Date_Return_To_Other_Store.push(Rent_Trade[3].toLocaleDateString('roc', {
                                        year: 'numeric',
                                        month: '2-digit',
                                        day: '2-digit'
                                    }))
                                } else {
                                    the_Date_Return_To_Bot.push(Rent_Trade[3].toLocaleDateString('roc', {
                                        year: 'numeric',
                                        month: '2-digit',
                                        day: '2-digit'
                                    }))
    ...
    User_Not_Return_For_Store: function (storeID, startTime, endTime) {
        return new Promise(function (resolve, reject) {
            UserOrderDB.find({
                ...
                if (err) return reject(err)
                let the_Date_User_Not_Return = UserOrders.map(Order => Order.orderTime.toLocaleDateString('roc', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit'
                }));
    ...
    Rent_UnLogRent_Return_For_Store: function (storeID, startTime, endTime) {
        return new Promise(function (resolve, reject) {
            tradeDB.find({
                ...
                let Trades = []
                trades.forEach(trade => {
                    let tradeTime = trade.tradeTime.toLocaleDateString('roc', {
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit'
                    });
        ...
        tradeDB.find({
            ...
            tradeDB.find({
                '$or': [{
                    ...
                    {
                        'tradeType.oriState': 1,
                        'tradeType.newState': 5
                    }
                ],
                'container.id': {
                    '$in': ContainersID_Store_Sign
                },
                'tradeTime': {
                    '$gt': the_first_day
                }
