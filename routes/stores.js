                ...
                keys.serverSecretKey((err, key) => {
                    ...
                    for (var i = 0; i < storeList.length; i++) {
                        if (storeList[i].active) {
                            ...
                            tmpOpening.sort((a, b) => { return a.close.day - b.close.day; });
                            tmpArr.push({
                                id: storeList[i].id,
...
router.post('/layoff/:id', regAsStoreManager, validateRequest, function(req, res, next) {
    ...
    process.nextTick(function() {
        User.findOne({ 'user.phone': toLayoff }, function(err, clerk) {
            if (err) return next(err);
            if (!clerk)
                return res.status(403).json({ code: 'E001', type: "userSearchingError", message: "No User: [" + toLayoff + "] Found", data: toLayoff });
...
function parseHistory(data, dataType, type, callback) {
    ...
    for (var i = 1; i < data.length; i++) {
        ...
        if (Math.abs(lastHistory.tradeTime - aHistory.tradeTime) <= 1000 || lastPhone !== thisPhone) {
